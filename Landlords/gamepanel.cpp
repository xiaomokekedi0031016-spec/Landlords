...
void GamePanel::gameControlInit()
{
    ...
    //todo...
    //处理玩家发射出的信号(用户在发牌阶段存储牌 >> 更新分发的扑克牌在窗口的显示)
...
void GamePanel::updatePlayerCards(Player *player)
{
    ...
    //取出展示扑克牌的区域
    int cardSpace = 20;
    QRect cardsRect = m_contextMap[player].cardRect;//获取玩家扑克牌的展示区域
    for(int i = 0; i < list.size(); ++i){
        CardPanel* panel = m_cardMap[list.at(i)];
        panel->show();
        panel->raise();
...
{
    // 1. 判断当前游戏状态是否是出牌阶段或者叫地主阶段, 如果不是, 直接返回
...
void GamePanel::mouseMoveEvent(QMouseEvent *ev)
{
    ...
    {
        QPoint pt = ev->pos();//获取鼠标在窗口中的坐标位置
...
void GamePanel::initCountDown()
{
    ...
    connect(userPlayer, &UserPlayer::startCountDown, this, [=](){
        if(m_gameCtl->getPendPlayer() != userPlayer &&
            m_gameCtl->getPendPlayer() != nullptr)
        {
